...
- `FR-002` — The AI response is strict JSON for exactly 7 days parsed by `PlanGeneratorParser.parseWithRawJson`; invalid JSON responds `400` with cause `AI_MALFORMED_JSON` (raw content truncated to 500 chars is logged).
...
- `FR-010` — Audit: each generation and confirmation records an audit log (`TRAINING_PLAN_GENERATED`, `TRAINING_PLAN_CONFIRMED`) following the same pattern as `AI_PROMPT_EXECUTED`.
