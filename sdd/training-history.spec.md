...
This Spec extends the existing contract: add `DAY_LOG` emissions with `dayLogId`, keep the query name, the `Date` UTC fields of `WeekLogReference` and the four-status enum (`pending | complete | skipped | rest` — there is no `none`). It distills `documents/plans/training-history/plan.md` into an executable contract. **Implemented and validated 2026-09-21** (`feat/training-history-calendar`); the plan was archived to `documents/legacy/plans/training-history/plan.md`. A follow-up extension adds **full `ExtraSession` objects** on every `CalendarDay` (`extraSessions`, see `FR-007`) so the frontend can render the day's extra-sessions directly; follow-up plan archived to `documents/legacy/plans/training-history-extra-sessions/plan.md` (Historical).
...
- `FR-007` — `workoutSessionId`, `extraSessionIds` and `extraSessions` on `CalendarDay`: for `WEEK_LOG`, resolved from the day's populated references (existing `resolveId` logic); for `DAY_LOG`, resolved from `dayLog.workoutSessionId` / `dayLog.extraSessionIds` (the day-log query **populates** `extraSessionIds`). `extraSessions: [ExtraSession]` (`@Field(() => [ExtraSession])`) carries the full `ExtraSession` objects (`id, userId, workoutSessionId, category, date, discipline, duration, intensityLevel, calories?, notes?`), defaulting to `[]` when a day has none. Populated entries whose references cannot be materialized (dangling refs) are skipped from `extraSessions` while their bare ids still appear in `extraSessionIds`. `extraSessionIds` remains the id-only list for compact consumption.
...
- The `DayLog` query may populate `extraSessionIds` to materialize full `ExtraSession` objects; also for that purpose the TrainingHistory module registers `ExtraSessionSchema` (`forFeature`) and the populate may be combined with `.populate('days.extraSessionIds')` and `.populate('extraSessionIds')`. The `extra-session` module itself stays untouched.
...
- `src/modules/routines/tracking/training-history/training-history.service.ts` (modify: inject `DayLog` model, validation per `FR-011`, `DAY_LOG` query with `populate('extraSessionIds')` + merge per `FR-004`/`BR-002`, `extraSessions` mapping per `FR-007`)
- `src/modules/routines/tracking/training-history/presentation/entities/training-history.entity.ts` (modify: add `dayLogId?: string` — `@Field(() => ID, { nullable: true })` — and `extraSessions?: ExtraSession[]` — `@Field(() => [ExtraSession])` — to `CalendarDay`)
- `src/modules/routines/tracking/training-history/training-history.module.ts` (modify: register `DayLog` schema and `ExtraSession` schema; drop the unused `WorkoutSession` `forFeature` entry)
...
- `TEST-001` — Unit (`training-history.service.spec.ts`): `WEEK_LOG` mapping — `isRest` day → `rest`; `pending | complete | skipped` passed through; in-range vs out-of-range days; `workoutSessionId`/`extraSessionIds` resolved after populate; `extraSessions` mapped to full `ExtraSession` objects; `weekLogReference` populated with `Date` fields. Proves `FR-003`, `FR-005`, `FR-006`, `FR-007`, `FR-008`, `BR-003`.
- `TEST-002` — Unit: `DAY_LOG` mapping — `type: DAY_LOG`, `dayLogId` set, status passed through `pending | complete | skipped`, session ids resolved, `extraSessions` mapped to full `ExtraSession` objects, `weekLogReference` null. Proves `FR-004`, `FR-005`, `FR-006`, `FR-007`, `FR-008`.
...
- `AC-006` — Every `CalendarDay` exposes `extraSessions` (defaults to `[]`) populated with full `ExtraSession` objects for both `WEEK_LOG` and `DAY_LOG` entries; `extraSessionIds` mirrors the same sessions by id.