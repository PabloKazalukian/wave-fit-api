...
- **`modifyPlan(id, comment)`** modifies a plan **not yet confirmed** (`confirmed: false`). It resends the **current plan** + the user context + the comment to the AI, and **overwrites the SAME** `TrainingPlan` document (updates `aiSnapshot`, metadata, dates and increments `version`). It throws `ConflictException` if the plan was already confirmed. Reuses `buildPlanSystemPrompt()` to guarantee the same JSON output format. Implementation history in `documents/legacy/plans/modify-plan/plan.md`.
